...
func Redis_projects_locations_instances_listHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateRedis_projects_locations_instances_listTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_parent_instances",
		mcp.WithDescription("Lists all Redis instances owned by a project in either the specified location (region) or all locations. The location should have the following format: * `projects/{project_id}/locations/{location_id}` If `location_id` is specified as `-` (wildcard), then all regions available to the project are queried, and the results are aggregated."),
		...
		mcp.WithString("pageToken", mcp.Description("The `next_page_token` value returned from a previous ListInstances request, if any.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Redis_projects_locations_instances_listHandler(cfg),
	}
}
